...
class Angle
{
	...
	public:
		...
		inline Angle( int angleDeg ) {    _angle = pmod( angleDeg * DtoR, TAU );} // angle in degrees *
		...
		inline Angle operator=( int angleDeg ){     _angle = pmod( angleDeg * DtoR, TAU ); return *this; } // angle in degrees *

		// ============================ ACCESSORS / MUTATORS
		inline angle_scalar_t normalize() { _angle = pmod( _angle, TAU ); return _angle; } // returns the angle in the range [0, TAU]
		...
		inline void setTo( double angle ){     _angle = pmod( angle, TAU ); } //           angle in radians
		inline void setTo( float  angle ){     _angle = pmod( angle, TAU ); } //           angle in radians
		inline void setTo( int angleDeg ){     _angle = pmod( angleDeg * DtoR, TAU ); } // angle in degrees *
		...
		inline float cosec() const { return 1.0f / sinf( _angle ); }

		explicit inline operator int()     const { return (int)_angle * RtoD; } // returns the angle in degrees
		...
		inline Angle operator+( const Vector2 &v ) const { return Angle( pmod( _angle + Angle( v )._angle, TAU )); } //   angle as a vector
		inline Angle operator+( double angle )     const { return Angle( pmod( _angle + angle, TAU )); } //               angle in radians
		inline Angle operator+( float  angle )     const { return Angle( pmod( _angle + angle, TAU )); } //               angle in radians
		inline Angle operator+( int angleDeg )     const { return Angle( pmod( _angle + ( angleDeg * DtoR ), TAU )); } // angle in degrees
		...
		inline bool isEqual(    const Angle &a ) const { return abs( _angle - a._angle ) < EPS; } //        NOTE : checks if the angles are equal
		inline bool isDiff(     const Angle &a ) const { return abs( _angle - a._angle ) > EPS; } //        NOTE : checks if the angles are not equal
		...
		inline bool isParallel( const Angle &a ) const { return abs( sinf( a._angle - _angle )) < EPS; } // NOTE : checks if this is parallel to a ( -90 or 90 degrees )
		inline bool isPerpend(  const Angle &a ) const { return abs( cosf( a._angle - _angle )) < EPS; } // NOTE : checks if this is perpendicular to a ( 0 or 180 degrees )
		inline bool isOpposite( const Angle &a ) const { return abs( a._angle - PI ) < EPS ; } //           NOTE : checks if this is opposite to a ( 180 degrees )
		inline bool isComplem(  const Angle &a ) const { return abs( a._angle + _angle ) < EPS ; } //       NOTE : checks if this is complementary to a ( a1 == -a2 )

		// Moves the angle by 45 degrees increments in the given direction ( might conflict with the stream operator )
		//inline Angle operator>>( int n ){ _angle = pmod( _angle + ( n * ( PI / 4.0f )), TAU ); return *this; } // rotate the angle clockwise
		//inline Angle operator<<( int n ){ _angle = pmod( _angle - ( n * ( PI / 4.0f )), TAU ); return *this; } // rotate the angle counter-clockwise
		...
		inline Angle getComplement() const { return Angle( pmod( -_angle, TAU )); } //      return the flips the angle around 0 ( ex : 37 -> -37)
		inline Angle getOpposite()   const { return Angle( pmod(  _angle + PI, TAU )); } // returns the opposite angle

		// increments / decrements the angle by 0.1 degrees
		inline Angle operator++(){ _angle = pmod( _angle + ( 0.1f * DtoR ), TAU ); return *this; }
		inline Angle operator--(){ _angle = pmod( _angle - ( 0.1f * DtoR ), TAU ); return *this; }

		// increments / decrements the angle by 0.1 degrees and returns the old value
		inline Angle operator++( int ){ Angle a = *this; _angle = pmod( _angle + ( 0.1f * DtoR ), TAU ); return a; }
		inline Angle operator--( int ){ Angle a = *this; _angle = pmod( _angle - ( 0.1f * DtoR ), TAU ); return a; }

		inline Angle operator+=( const Angle &a ){   _angle = pmod( _angle + a._angle, TAU );            return *this; }
		inline Angle operator+=( const Vector2 &v ){ _angle = pmod( _angle + Angle( v )._angle, TAU );   return *this; } // angle as a vector
		inline Angle operator+=( double angle ){     _angle = pmod( _angle + angle, TAU );               return *this; } // angle in radians
		inline Angle operator+=( float  angle ){     _angle = pmod( _angle + angle, TAU );               return *this; } // angle in radians
		inline Angle operator+=( int angleDeg ){     _angle = pmod( _angle + ( angleDeg * DtoR ), TAU ); return *this; } // angle in degrees *
		...
		// ============================ STATIC FUNCTIONS
		static inline angle_scalar_t normalizeRad( angle_scalar_t angle ) { return pmod( angle, TAU ); } //    returns the angle in the range [0, TAU]
		static inline angle_scalar_t normalizeDeg( angle_scalar_t angle ) { return pmod( angle, 360.0f ); } // returns the angle in the range [0, 360]
		...
		static inline double toDouble( Vector2 v ){    return atan2f( v.y, v.x ); } //             angle as a vector
		...
		static inline float toFloat( Vector2 v ){    return atan2f( v.y, v.x ); } //               angle as a vector
		...
		static inline int toInt( Vector2 v ){    return ( int )( atan2f( v.y, v.x ) * RtoD ); } // angle as a vector
